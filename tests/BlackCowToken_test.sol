...
import "../contracts/BlackCowToken.sol"; // INPUT by minias

contract BlackCowTokenTest is BlackCowToken { // INPUT by minias
    ...
    function testTokenInitialValues() public {
        Assert.equal(name(), "BlackCow", "token name did not match"); // INPUT by minias
        Assert.equal(symbol(), "BLC", "token symbol did not match"); // INPUT by minias
        Assert.equal(decimals(), 18, "token decimals did not match");
        Assert.equal(totalSupply(), 0, "token supply should be zero"); // INPUT by minias
    }
